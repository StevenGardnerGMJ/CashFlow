...
//        view.addSubview(webView)
    ...
    // 请求
    func sendRequestURL(url:URLRequest, web:WKWebView) {
...
//        print(" ====A==B==C==D===== ")
        ...
        webView.evaluateJavaScript("document.body.innerText") { (result, error) in
            ...
            case self.wkweb_dow:
                print("=========== dow ============")
                let body = result as! String
                let cbody = body.subString(from: 39, to: 182)
                self.marketStringToArr(cbody: cbody, mName: "dow")
                
            case self.wkweb_sandp:
                print("========== sandp ============")
                let body = result as! String
                let cbody = body.subString(from: 39, to: 192)
...
//            print(Orstr)
            ...
            if PEarr.count >= 5  {
                ...
                let marketDate = PEarr[4] //2020-08-03
                
                let defaults = UserDefaults.standard
                // 股市指数 中国市场
                defaults.set(Vstr, forKey: "\(mName)marketValue")
                // PE值
                defaults.set(marketPE, forKey: "\(mName)marketPE")
                // 日期
                defaults.set(marketDate, forKey: "\(mName)marketDate")
                // 数据来源
                defaults.set(Orstr, forKey: "\(mName)marketOrigin")
...
//        print(cbody.count)
        ...
        if marketArr.count >= 7 {
            ...
            } else if Varr.count >= 3 && PEarr.count >= 2 && Orarr.count >= 2 {
