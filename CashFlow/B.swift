...
class B: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    let total = ["总收入","总支出"]
    let income = ["总收入","主动收入","被动收入","工资收入","银行存款","股票红利","房产租金","商业现金","其他B"]
    let incomeOther = ["版权收入","专利收入","意外收入","小生意"]
    let expenditure = ["总支出","税金","房贷","车贷","教育贷款","信用卡","维修费","医疗支出","花呗类","意外支出","小孩支出","银行贷款支出","取暖","物业","停车","燃油","其他B"]
